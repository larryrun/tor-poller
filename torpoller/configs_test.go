...
func TestReadConfig(t *testing.T) {
	...
	assert.Equal(t, 1, config.Items[0].Extra["start-season"])
	assert.Equal(t, 1, config.Items[0].Extra["start-episode"])
	assert.Equal(t, "name2", config.Items[1].Name)
	assert.Equal(t, "type2", config.Items[1].Type)
	assert.Equal(t, "info2", config.Items[1].Info)
	assert.Equal(t, 2, config.Items[1].Extra["start-season"])
	assert.Equal(t, 2, config.Items[1].Extra["start-episode"])
}